...
def merge_dbfl(db_d, fl_d, col_dict, ow_flag):
    ...
    if 'SKIP the column'      in col_dict_out.values():
        skip         = [k    for k, v in col_dict_out.items() if v == 'SKIP the column']  # find the keys in the "col_dict_out" dictionary that correspond to values 'SKIP the column'
        col_dict_out = {k: v for k, v in col_dict_out.items() if k not in skip}           # delete the items from the "col_dict_out" dictionary
        ow_flag  = {k: v for k, v in  ow_flag.items() if k not in skip}           # delete the items from the "col_dict_out" dictionary.
    ...
    ow_flag = {col_dict_out[k]: v for k, v in ow_flag.items()}   # rename keys in the dictionary with "overwrite non-NaN" flags using their database counterparts
        
    for i in range(0,fl_d1.shape[1]):
        tmp = fl_d1.iloc[:,i]
        db_d1.update(tmp, overwrite = ow_flag.get(tmp.name) )             # replace nan elements in db by finite values in file
